 ...
 */
short SimpleDigitalPin::invertState(short st) {
 ...
 */
void SimpleDigitalInputPin::setCurrentPinMode(short pm) {
    currentPinMode = pm;
    pulse.reset();
    setLogicalState(LOW);
 }
 ...
 */
void SimpleDigitalInputPin::processPinState(long tm, short priorState) {
...
void SimpleDigitalInputPin::determinePinState() {
   ...
   short priorState = state;
   
   // get curren system time
   long tm = millis();

   // read the input pin:
   short reading = digitalRead(pinNumber);
...
bool SimpleDigitalInputPin::readInputPulseMode() {
   ...
   if (hasChanged()) {
      ...
      unsigned short pulseDur  = getLastPulse().duration;
